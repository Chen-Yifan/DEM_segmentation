...
def unet_shirui(channels=1, lmbda=1e-6, drop=0.45, init=None, n_filters=32, output_mode='sigmoid', 
    ...
    if output_mode == 'softmax':
        ...
        model.compile(loss=L.keras_lovasz_hinge, metrics=[iou_label(), per_pixel_acc(), accuracy()], optimizer=optimizer)
...
def unet_rgl(channels=1, learn_rate=1e-4, n_filters=64, output_mode='sigmoid', lmbda=1e-6):
    inputs = Input((128, 128, channels))
...
def unet(channels=1, learn_rate=1e-4, n_filters=64, activation_='relu', optloss='bce', output_mode='sigmoid'):
    inputs = Input((128, 128, channels))
    conv1 = Conv2D(n_filters, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(inputs)
    conv1 = Conv2D(n_filters, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    conv2 = Conv2D(n_filters*2, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(pool1)
    conv2 = Conv2D(n_filters*2, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    conv3 = Conv2D(n_filters*4, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(pool2)
    conv3 = Conv2D(n_filters*4, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    conv4 = Conv2D(n_filters*8, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(pool3)
    conv4 = Conv2D(n_filters*8, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv4)
    drop4 = Dropout(0.5)(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(drop4)   
    conv5 = Conv2D(n_filters*16, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(pool4)
    conv5 = Conv2D(n_filters*16, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv5)
    drop5 = Dropout(0.5)(conv5) 
    up6 = Conv2D(n_filters*8, 2, activation=activation_, padding='same',
                 kernel_initializer='he_normal')(UpSampling2D(size=(2, 2))(drop5))
    merge6 = concatenate([drop4, up6], axis=3)
    conv6 = Conv2D(n_filters*8, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(merge6)
    conv6 = Conv2D(n_filters*8, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv6)   
    up7 = Conv2D(n_filters*4, 2, activation=activation_, padding='same',
                 kernel_initializer='he_normal')(UpSampling2D(size=(2, 2))(conv6))
    merge7 = concatenate([conv3, up7], axis=3)
    conv7 = Conv2D(n_filters*4, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(merge7)
    conv7 = Conv2D(n_filters*4, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv7)   
    up8 = Conv2D(n_filters*2, 2, activation=activation_, padding='same',
                 kernel_initializer='he_normal')(UpSampling2D(size=(2, 2))(conv7))
    merge8 = concatenate([conv2, up8], axis=3)
    conv8 = Conv2D(n_filters*2, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(merge8)
    conv8 = Conv2D(n_filters*2, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv8)   
    up9 = Conv2D(n_filters, 2, activation=activation_, padding='same',
                 kernel_initializer='he_normal')(UpSampling2D(size=(2, 2))(conv8))
    merge9 = concatenate([conv1, up9], axis=3)
    conv9 = Conv2D(n_filters, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(merge9)
    conv9 = Conv2D(n_filters, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv9)
    conv9 = Conv2D(2, 3, activation=activation_, padding='same',
                  kernel_initializer='he_normal')(conv9)
