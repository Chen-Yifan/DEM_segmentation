...
def data_gen(img_folder, mask_folder, batch_size):
    ...
    while (True):
        ...
        mask = np.zeros((batch_size, 256, 256, NUMBER_OF_CLASSES), dtype=np.uint8)
...
#     random.shuffle(n)
    ...
    img = np.zeros((len(n), 256, 256, 5)).astype('float')
    mask = np.zeros((len(n), 256, 256, NUMBER_OF_CLASSES), dtype=np.uint8)

    for i in range(len(n)): #initially from 0 to 16, c = 0. 
        test_img_0 = np.load(img_folder+'/'+n[i])/360. #normalization:the range is about -100 to 360
        #train_img =  cv2.resize(train_img, (256, 256))# Read an image from folder and resize
